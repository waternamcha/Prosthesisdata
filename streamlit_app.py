...
# ---------------------------------------------------------
def create_html():
    # Helper: จัดการ Date และคำนวณอายุ
    dob = st.session_state.dob.strftime('%d/%m/%Y')
    age_calc = date.today().year - st.session_state.dob.year
    
    # Helper: จัดการ List และวงเล็บ (ระบุ...)
    def fmt_multi(val_list, ot_key):
        # แปลงเป็น string ก่อนเสมอเพื่อป้องกัน error
        if not isinstance(val_list, list):
            val_list = [str(val_list)]
            
        txt = ", ".join(val_list)
        # เช็คว่ามีคำว่า "Other" หรือ "อื่นๆ" หรือไม่
        if any(x in ["Other", "อื่นๆ"] for x in val_list):
            other_text = st.session_state.get(ot_key, '-')
            txt += f" ({other_text})"
        return txt if txt else "-"

    # เตรียมข้อมูลสำหรับใส่ในตาราง
    comorb_txt = fmt_multi(st.session_state.comorbidities, 'comorb_ot')
    
    surg_det_txt = "-"
    if st.session_state.surgery == 'ใช่':
        surg_det_txt = ", ".join(st.session_state.surg_details)
    ...
    # กำหนดสี TUG Status
    tug_color = '#C0392B' if st.session_state.tug_avg >= 13.5 else '#27AE60'
    ...
    <head>
        ...
        <style>
            ...
            /* TUG Box Style */
            .tug-box {{ text-align: center; border: 2px solid #1F618D; padding: 15px; margin-top: 20px; border-radius: 10px; background: #f0f8ff; page-break-inside: avoid; }}
            
            @media print {{ body {{ padding: 0; }} .section, .tug-box {{ break-inside: avoid; }} }}
        </style>
    </head>
    <body>
        ...
        <div class="tug-box">
            <div style="font-weight:bold; color:#1F618D; font-size: 1.2em;">ผลการทดสอบ Timed Up and Go (TUG)</div>
            <div style="font-size: 2.5em; font-weight: bold; margin: 10px 0; color: #333;">{st.session_state.tug_avg:.2f} วินาที</div>
            <div style="font-weight: bold; font-size: 1.2em; color: {tug_color}; margin-bottom: 5px;">
                {st.session_state.tug_status}
            </div>
...
# ---------------------------------------------------------
# เรียกใช้ฟังก์ชัน และ สร้างปุ่ม Download (วางทับส่วนเดิม)
# ---------------------------------------------------------
html_data = create_html()

# แสดง Header และปุ่ม Download มุมขวาบน
